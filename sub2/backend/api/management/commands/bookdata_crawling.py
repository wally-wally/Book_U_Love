...
def get_keyword_distribution(sentence):
...
def get_book_data(bookID):
    ...
    response_data["description"], response_data["contents"] = get_more_info(bookID)
    # return response_data
    
    # description에서 단어 개수 분포 추출
    if response_data["description"] == '': # description이 없는 경우 contents(목차) 정보로 단어 개수 분포 추출
        response_data["keywordDistribution"] = get_keyword_distribution(response_data["contents"])
    else: # description이 있는 경우
        response_data["keywordDistribution"] = get_keyword_distribution(response_data["description"])
    return response_data
...
for cat_num in cat_nums:
    ...
    print(f'{total_book_cnt}권 중 420권의 도서 데이터를 가져옵니다.')

    books_list = []
    for i in range(1, 420, 21):
